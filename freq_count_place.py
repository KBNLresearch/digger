...
def exec_query(ppn, place, date, atype):
    done = False

    while not done:
        try:
            date_start = date_end = date
            sru = SRU_QUERY % (place, ppn, date_start, date_end, atype, '0')
        ...
        for place in WANTED_PLACES:
            for date in WANTED_RANGES:
                for atype in WANTED_TYPES:
                    result.append((ppn, place, date, atype, exec_query(ppn, place, date, atype)))
