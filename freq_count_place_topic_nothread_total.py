...
def exec_query(topic, ppn, date, atype):
    done = False

    while not done:
        sru = SRU_QUERY % (topic, ppn, date, date, atype, '0')
        try:
...
def read_topics(fname='categories_all.txt'):
...
if __name__ == '__main__':
    ...
    for ppn in WANTED_PPNS:
        for topic in WANTED_TOPICS:
            with open(OUTPUT_DIR + "/" + topic + "_total.csv", 'a') as fh:
